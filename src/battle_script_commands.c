...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) >= GetPlayerLevelCap())
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) && GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) < GetPlayerLevelCap())
            {
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_ON_DAMAGE_ABILITIES: // Such as abilities activating on contact (Effect Spore, Rough Skin, etc.).
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_PLUS_MINUS_STATUS:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerTarget].hp != 0
             && gBattlerAttacker != gBattlerTarget
             && GetBattlerSide(gBattlerAttacker) != GetBattlerSide(gBattlerTarget)
             ...
             && (Random() % 5) == 0)
            {
                if (gBattleMons[gBattlerAttacker].ability == ABILITY_PLUS)
                {
                    gBattleCommunication[MOVE_EFFECT_BYTE] = MOVE_EFFECT_BURN;
                    BattleScriptPushCursor();
                ...
                else if (gBattleMons[gBattlerAttacker].ability == ABILITY_MINUS)
                {
